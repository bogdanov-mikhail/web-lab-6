...
class Server(threading.Thread):
    ...
    def __init__(self, hostname='localhost', port=6334):
        super().__init__()
        self.connections = []
        self.host = hostname
        self.port = port
...
class Connection(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            if message:
                logging.debug(f"Client '{self.client_name}' send message '{message}'")
                self.server.broadcast(message, self.client_name)
            else:
                logging.debug(f"Client '{self.client_name}' closed the connection")
                self.sc.close()
